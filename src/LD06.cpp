#include "ld06/LD06.hpp"

static const uint8_t crc_table[256] =
{
	
};

void LD06_init(LD06 *ld06, uint8_t (*readFunc)(), void (*receivedCallback)(LD06 *))
{
    ld06->readFunc = readFunc;
    ld06->receivedCallback = receivedCallback;

    ld06->index = 0;
}

void LD06_read(LD06 *ld06)
{
    if(ld06->readFunc == NULL)
    {
        return;
    }

    ld06->data[ld06->index] = ld06->readFunc();

    if(ld06->index == 0){
        ld06->header0 = ld06->data[0];
        ld06->crc = 0;
        if(ld06->header0 != LD06_HEADER0)
        {
            ld06->index = 0;
            return;
        }
    }
    else if(ld06->index == 1)
    {
        ld06->header1 = ld06->data[1];
        if(ld06->header1 != LD06_HEADER1)
        {
            ld06->index = 0;
            return;
        }
    }

    if(ld06->index == LD06_DATA_SIZE - 1)
    {
        if(ld06->crc == ld06->data[LD06_DATA_SIZE - 1]){
            ld06->speed = (uint16_t)((ld06->data[3] << 8) | ld06->data[2]) / 180.0f * M_PI_F;
            ld06->start_angle = (uint16_t)((ld06->data[5] << 8) | ld06->data[4]) / 100.0f  / 180.0f * M_PI_F;
            ld06->end_angle = (uint16_t)((ld06->data[43] << 8) | ld06->data[42]) / 100.0f  / 180.0f * M_PI_F;
            ld06->timestamp = (ld06->data[45] << 8) | ld06->data[44];

            float d_angle = ld06->end_angle - ld06->start_angle;
            while (d_angle >= 2 * M_PI_F)
            {
                d_angle -= 2 * M_PI_F;
            }
            while (d_angle < 0)
            {
                d_angle += 2 * M_PI_F;
            }
            ld06->step = d_angle / (LD06_NUM_POINTS - 1);
            
            for (size_t i = 0; i < LD06_NUM_POINTS; i++)
            {
                ld06->angle[i] = ld06->start_angle + i * ld06->step;
                ld06->distance[i] = (ld06->data[7 + 3 * i] << 8) | ld06->data[6 + 3 * i];
                ld06->confidence[i] = ld06->data[8 + 3 * i];
            }
            
            if(ld06->receivedCallback != NULL){
                ld06->receivedCallback(ld06);
            }
        }
        ld06->index = 0;
        return;
    }
    else
    {
        ld06->crc = crc_table[(ld06->crc ^ ld06->data[ld06->index]) & 0xFF];
    }

    ld06->index ++;
}

